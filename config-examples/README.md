...
本桥接只提供本机 stdio，插件使用固定端点 `ws://localhost:9753/plugin`。桥接分别显式监听 `127.0.0.1:9753` 与 `[::1]:9753`，共享认证，不绑定通配地址。同一时刻只启动一个宿主的桥接实例，连接一个 **Figma Canvas Writer 2** 插件。此目录不代表上述客户端均已通过真实 Figma 读写验收。

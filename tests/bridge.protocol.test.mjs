...
const context = { sessionId: 'test-session', pageId: '1:2', pageName: '测试页', fileName: '模拟文件', editorType: 'figma' };
...
async function rawPeer(f, origin = 'null', hostname = '127.0.0.1') {
  const p = { messages: [], sendSeq: 0, recvSeq: 0 };
  ...
  for (const version of ['2024-11-05', '2025-03-26', '2025-06-18', '2025-11-25']) {
    ...
    try {
      ...
      for (const t of list.result.tools) {
        if (t.name === 'figma_canvas_status') continue;
        assert(t.inputSchema.required.includes('sessionId'));
        assert(t.inputSchema.required.includes('pageId'));
      }
...
test('real stdio: JSON-RPC errors, initialization state, size cap and stream recovery', async () => {
  ...
  try {
    ...
    const wrong = await f.tool('figma_get_context', { sessionId: context.sessionId, pageId: context.pageId, limit: 1.5 });
...
test('real HMAC peer: schemas preserve text omission/STAR/props/paint alpha and errors, reject stale or extra args', async () => {
  ...
  try {
    ...
    const a = { sessionId: context.sessionId, pageId: context.pageId, operationId: 'font-only', nodeId: '1:3', fontSize: 24 };
    ...
    const create = f.tool('figma_create_node', { sessionId: context.sessionId, pageId: context.pageId, operationId: 'star', type: 'STAR', props: { fills: [{ type: 'SOLID', color: { r: 1, g: 0, b: 0, a: 0.3 }, visible: false }] } });
    ...
    const invalids = [
      ...
      ['figma_create_node', { sessionId: context.sessionId, pageId: context.pageId, operationId: 'dup', type: 'FRAME', width: 5, props: { width: 6 } }],
    ];
...
test('real HMAC peer: tampering disconnects with unknown write state; replayed frame cannot settle a new request', async () => {
  ...
  try {
    await f.init(); const p = await rawPeer(f);
    const call = f.tool('figma_create_node', { sessionId: context.sessionId, pageId: context.pageId, operationId: 'tamper-op', type: 'RECTANGLE' });
    const cmd = await p.command();
    ...
    const b = await rawPeer(f);
    assert(b.ack.ok);
    const read = f.tool('figma_get_context', { sessionId: context.sessionId, pageId: context.pageId });
    ...
    const read2 = f.tool('figma_get_context', { sessionId: context.sessionId, pageId: context.pageId });
    await b.command();
...
test('real peer timeout: write returns unknown; late reply does not replay write, getOperation reconciles same id', async () => {
  ...
  try {
    await f.init(); const p = await rawPeer(f);
    const call = f.tool('figma_delete_node', { sessionId: context.sessionId, pageId: context.pageId, operationId: 'late-delete', nodeId: '1:3' });
    const cmd = await p.command();
    const result = content(await call);
    ...
    const reconcile = f.tool('figma_get_operation', { sessionId: context.sessionId, pageId: context.pageId, operationId: 'late-delete' });
    const query = await p.command();
    assert.equal(query.command, 'getOperation'); assert.deepEqual(query.params, { operationId: 'late-delete' });
...
test('real stdio: cancellation suppresses the response without replaying an in-flight write', async () => {
  ...
  try {
    await f.init(); const p = await rawPeer(f);
    f.send({ jsonrpc: '2.0', id: 'cancel-write', method: 'tools/call', params: { name: 'figma_delete_node', arguments: { sessionId: context.sessionId, pageId: context.pageId, operationId: 'cancel-op', nodeId: '1:3' } } });
    const cmd = await p.command();
...
test('real HMAC peer: oversized UTF-8 command is not_started and does not consume frame sequence', async () => {
  ...
  try {
    await f.init();
    const args = { sessionId: context.sessionId, pageId: context.pageId, operationId: 'large-text', type: 'TEXT', text: '测'.repeat(100000) };

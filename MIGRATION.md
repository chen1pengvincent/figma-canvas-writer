...
|---|---|---|
| 连接/鉴权/协议故障路径 | ✅ | ✅ 真机配对/重连/切页重连/多文件切换 |
| 基础编辑（创建/修改/删除/文本） | ✅ | ✅ 真机创建/修改/回读/删除/去重/冲突 |
| 跨页世代/对账 | ✅（含回归） | ✅ 真机切页 + pageRevision 递增 + 旧目标拒绝 + 对账可用 |
| 资源导入导出/截图/产物 | ✅ | ✅ 截图内联、PNG/SVG 导出落盘、SHA-256 一致、真图导入回填 |
| 语义读取/续读/设计上下文 | ✅ | ✅ queryNodes/get_text_runs（部分动作由 core 组覆盖） |
| 层级/矢量/布局/富文本/视觉 | ✅ | ✅ 多边形/矢量路径/布尔/Auto Layout/clone/DROP_SHADOW/分角圆角/区间样式 |
| 变量/样式/组件 | ✅ | ✅ 集合创建/变量/赋值/模式/解析 + 样式创建应用 + 组件/实例 |
| 原型 reactions/batch | ✅ | ✅ reactions 设置/清除、batch 成功/失败逐项状态（原型点击人验收另行执行） |
| Motion/MP4/Shader | ✅ | ✅ 手动关键帧轨道、动画样式 6 个、MP4 真实导出（3233B）；Shader 列表 API 通（样本 0，如实记录） |
| FigJam | ✅ | ✅ 便笺/形状/连接线/更新/读取，原生可编辑 |
| Slides | ✅ | ✅ 结构读取/行创建/内容节点/更新；⚠️ createSlide 在 Figma Desktop 126.8.18 返回 "Cannot create node with type SLIDE"（裸调用与 (0,0) 均受限，已如实报 EDITOR_LIMITATION，用文件默认幻灯片验证内容能力） |
| 团队库发现导入 | ✅ | ⏳ 未验收（需用户已在 Figma UI 启用库的文件，待具备条件后补验） |
